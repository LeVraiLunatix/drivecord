 ...
 *
 *   application/x-discloud-item   →  payload is a JSON DragPayload
 *
...
export const DISCLOUD_ITEM_MIME = "application/x-discloud-item";
...
export function setItemDrag(dt: DataTransfer, item: DriveItem): void {
  ...
  dt.setData(DISCLOUD_ITEM_MIME, JSON.stringify(payload));
...
export function readItemDrag(dt: DataTransfer): DragPayload | null {
  const raw = dt.getData(DISCLOUD_ITEM_MIME);
  if (!raw) return null;
  try {
...
export function isInternalItemDrag(dt: DataTransfer | null): boolean {
  if (!dt) return false;
  return Array.from(dt.types).includes(DISCLOUD_ITEM_MIME);
}
